...
class PD_Transliterator():
    ...
    def fit(self):
        ...
        self.coef_            = np.load('%s/models/uh_coef.npy' %dist_dir, allow_pickle=True, encoding='latin1')[0]
        self.classes_         = np.load('%s/models/uh_classes.npy' %dist_dir, allow_pickle=True, encoding='latin1')[0]
        self.vectorizer_      = np.load('%s/models/uh_sparse-vec.npy' %dist_dir, allow_pickle=True, encoding='latin1')[0]
        ...
        self.letter_set = set([chr(i) for i in range(int("0x0621", 16), int("0x063b", 16))]) | \
                          set([chr(i) for i in range(int("0x0641", 16), int("0x064b", 16))]) | \
                          set([chr(i) for i in range(int("0x0674", 16), int("0x06d4", 16))])

        #initialize punctuation map table
        self.punkt_tbl = dict()
        with open('%s/mapping/punkt.map' %dist_dir, encoding='utf-8') as punkt_fp:
        ...
        with open('%s/mapping/urdu_urdu.map' %dist_dir, encoding='utf-8') as nu_fp:
            for line in nu_fp:
                src, trg = line.split()
    ...
    def feature_extraction(self, letters):
        ...
        for i in range(ngram, len(context)-ngram):
            ...
            bigrams   = [p + b'|' + q for p,q in zip(unigrams[:-1], unigrams[1:])]
            trigrams  = [p + b'|' + q + b'|' + r for p,q,r in zip(unigrams[:-2], unigrams[1:], unigrams[2:])]
            quadgrams = [p + b'|' + q + b'|' + r + b'|' + s for p,q,r,s in zip(unigrams[:-3], unigrams[1:], unigrams[2:], unigrams[3:])]
    ...
    def matra_norm(self, line):
        ...
        line = re.sub(u'\u06cc\u0670([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0627\\1', line)  # khada-alif
        line = re.sub(u'\u06d2\u0670([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0627\\1', line)  # khada-alif
        line = re.sub(u'\u0670\u06cc([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0627\\1', line)  # khada-alif
        line = re.sub(u'\u0670\u06d2([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0627\\1', line)  # khada-alif
        line = line.replace(u'\u0627\u0670', u'\u0622') #khada-alif
        line = line.replace(u'\u0622\u0670', u'\u0622') #khada-alif
        line = line.replace(u'\u0670', u'\u0627')   #khada-alif
        line = re.sub(u'\u0627\u064b([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0646\\1', line)  # double-zabar 
        line = re.sub(u'\u064b\u0627([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0646\\1', line)  # double-zabar
        line = re.sub(u'\u064b([^\u0621-\u063a\u0641-\u064a\u0674-\u06d3])', u'\u0646\\1', line)  # double-zabar
        line = line.replace(u'\u064b', '')  #double-zabar
        line = line.replace(u'\u0644\u0644\u06c1', u'\u0644\u0644\u0627\u06c1')  #llh > llah
        line = re.sub(u'([^\u06a9\u06af\u0686\u062c\u0679\u0688\u062a\u062f\u067e\u0628\u0691])\u06be', u'\\1\u06c1', line)
